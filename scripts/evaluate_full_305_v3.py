#!/usr/bin/env python3
"""
Full 305 v3 Fair Evaluation
===========================
Evaluate on dataset_test_classification_full.json using v3 protocol.

Key differences from canonical clean evaluation:
- Includes 94 fallback samples (label from filename prefix)
- Reports canonical vs fallback separately
"""
...
# Same configuration as evaluate_v3.py
@dataclass
class EvalConfig:
    model_path: str = "D:/dataset/models_hf/models--Qwen--Qwen3-VL-2B-Instruct/snapshots"
    adapter_path: Optional[str] = None
    test_data_path: str = "D:/dataset/data/dataset_test_classification_full.json"
    images_dir: str = "D:/dataset/sample_images"
    output_dir: str = "D:/dataset/full_305_v3_results"
...
def evaluate_sample(model, processor, process_vision_info, item: dict, 
    ...
    label_source = classification.get('label_source', item.get('label_source', 'unknown'))
    ...
    # Get ground truth from classification when available.
    gt_disaster_type = classification.get('disaster_type')
    # Fall back to assistant JSON for older exported full files.
...
def main():
    ...
    parser.add_argument("--model_path", default="D:/dataset/models_hf/models--Qwen--Qwen3-VL-2B-Instruct/snapshots")
    parser.add_argument("--adapter_path", default="D:/dataset/ablation_canonical_output/scale_100pct/final_adapter")
    parser.add_argument("--output_dir", default="D:/dataset/full_305_v3_results")
    ...
    # Count by label_source
    canonical_count = sum(1 for item in test_data if item.get('classification', {}).get('label_source', item.get('label_source')) == 'canonical')
    fallback_count = sum(1 for item in test_data if item.get('classification', {}).get('label_source', item.get('label_source')) == 'filename_fallback')
